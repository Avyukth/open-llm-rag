...
class OllamaFactory(ModelFactory):
    ...
    def _check_ollama_service(self):
        try:
            ...
            logger.info(
                f"Ollama service checked. Model '{self.model_name}' is available."
            )
        except requests.RequestException as err:
            logger.error(
                f"Failed to connect to Ollama service at {self.base_url}: {str(err)}"
            )
            raise ConnectionError(
                f"Unable to connect to Ollama service at {self.base_url}."
            ) from err
